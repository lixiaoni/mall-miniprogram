...
class JumpMiniprogram {
   ...
   * pathSign:路径判断  1首页  2商品详情页面  3工作台 4小程序码页面  5个人中心页面 6供应商资料页面 
   */
  toMiniProgram(params, pathSign) {
    ...
    if (pathSign == 3) {
      ...
      this.path = this.pathData.storeIconUrl + "?storeId=" + params.storeId
    } else if (pathSign == 5) {
      this.path = this.pathData.userUrl + "?storeId=" + params.storeId
    } else if (pathSign == 6) {
      this.path = this.pathData.informationUrl + "?status=0&send=&accept=" + params.storeId+"&remark="
    }else {
